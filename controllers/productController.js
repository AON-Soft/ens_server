...
exports.createProduct = catchAsyncError(async (req, res, next) => {
  ...
  // New validation for points
  if (price >= 2 && (!points || points < 0.5)) {
    return next(
      new ErrorHandler(
        'Products with price 2 or above must have at least 0.50 points',
        400,
      ),
    )
  }
  ...
  try {
    ...
    let productData = {
      ...
      description,
      price,
      points: price < 2 ? 0 : points, // Set points to 0 if price is less than 2
      images,
...
exports.getAllProductsByUser = catchAsyncError(async (req, res) => {
  ...
  const apiFeature = new ApiFeatures(
    Product.find({ shop: req.params.id, status: 'live' })
      .select('-__v')
      .populate({
        path: 'categoryId',
        select: 'image name',
...
exports.getProductsByCategory = catchAsyncError(async (req, res) => {
  ...
  const apiFeature = new ApiFeatures(
    Product.find({ categoryId, status: 'live' })
      .select('-__v')
      .populate({
        path: 'categoryId',
        select: 'image name',
...
exports.getProductsByBrand = catchAsyncError(async (req, res) => {
  ...
  const apiFeature = new ApiFeatures(
    Product.find({ brandId, status: 'live' })
      .select('-__v')
      .populate({
        path: 'categoryId',
        select: 'image name',
